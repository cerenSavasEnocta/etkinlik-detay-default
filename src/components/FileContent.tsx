...
export function FileContent({ onComplete }: FileContentProps) {
  ...
  return (
    <div className="relative px-6 py-6 flex flex-col items-center" style={{ backgroundColor: '#f9f9f9' }}>
      {/* Ana Container */}
      <div className="w-full" style={{ maxWidth: '1100px', maxHeight: '67vh', minHeight: '67vh', overflowY: 'auto' }}>
        {/* Banner Görseli */}
        ...
        <div className="bg-white rounded border border-gray-200 p-3 flex items-center justify-between mb-6">
          ...
          >
            {downloadState === 'idle' && <Download className="w-5 h-5 text-gray-900" />}
            {downloadState === 'downloading' && <Loader2 className="w-5 h-5 text-gray-900 animate-spin" />}
            {downloadState === 'completed' && <Check className="w-5 h-5 text-green-600" />}
            <span className="text-sm text-gray-900">İndir</span>
          </button>
        </div>
